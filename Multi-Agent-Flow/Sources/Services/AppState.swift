...
private final class WorkbenchEntryReplyTracker {
    let messageID: UUID
    ...
    init(
        ...
        firstReplyAt: Date? = nil,
        fullWorkflowAt: Date? = nil
    ) {
        self.messageID = messageID
        self.streamingContent = streamingContent
        self.firstChunkAt = firstChunkAt
        self.firstReplyAt = firstReplyAt
        self.fullWorkflowAt = fullWorkflowAt
    }
}

private struct WorkbenchSubmissionSession {
    let startedAt: Date
    let context: WorkbenchThreadContext
    let taskID: UUID
    let userRuntimeEventID: String?
}

private struct WorkbenchSubmissionRouting {
    let entryAgentNodes: [WorkflowNode]
    let leadNode: WorkflowNode
    let leadAgent: Agent
...
private struct WorkbenchRecordQuery {
    let workflowID: UUID
    let threadID: String?
    let requiresWorkbenchChannel: Bool

    func matches(metadata: [String: String]) -> Bool {
        if requiresWorkbenchChannel,
           metadata[WorkbenchMetadataKey.channel] != "workbench" {
            return false
        }

        guard metadata[WorkbenchMetadataKey.workflowID] == workflowID.uuidString else {
            return false
        }

        return threadID == nil || resolvedWorkbenchThreadID(from: metadata) == threadID
    }
}

private struct WorkbenchThreadHistory {
...
class AppState: ObservableObject {
    ...
    var openClawRuntimeControlPlaneSummary: String {
        let currentEntry = currentOpenClawRuntimeControlPlaneEntry
        if currentEntry.gate == .publish,
           let executeEntry = openClawRuntimeControlPlane.first(where: { $0.gate == .execute }),
           executeEntry.status == .ready,
           executeEntry.detail != currentEntry.detail {
            return executeEntry.detail
        }
        return currentEntry.detail
    }

    var openClawRuntimeControlPlaneSecondarySummary: String? {
        let primary = openClawRuntimeControlPlaneSummary.trimmingCharacters(in: .whitespacesAndNewlines)

        if let blockingMessage = openClawRunControlledBlockingMessage?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !blockingMessage.isEmpty,
           blockingMessage != primary {
            return blockingMessage
        }

        if let compactDiagnostic = openClawLatestRuntimeSyncCompactDiagnostic?
            .trimmingCharacters(in: .whitespacesAndNewlines),
           !compactDiagnostic.isEmpty,
           compactDiagnostic != primary {
    ...
    private func openClawRuntimeControlPlaneSnapshot() -> ProjectOpenClawControlPlaneSnapshot {
        ...
        return ProjectOpenClawControlPlaneSnapshot(
            ...
            secondarySummary: openClawRuntimeControlPlaneSecondarySummary,
            updatedAt: Date()
        )
    }
    ...
    private func syncCurrentProjectFromManagers() {
        ...
        if var confirmedProject = appliedProjectSnapshot, confirmedProject.id == project.id {
            ...
            confirmedProject.runtimeState.workbenchThreadStates = project.runtimeState.workbenchThreadStates
    ...
    private func clearTransientRuntimeState() {
        ...
        if var project = currentProject {
            markActiveWorkbenchThreadStatesFailed(
                in: &project.runtimeState,
                reason: "OpenClaw disconnected before the thread completed."
            )
    ...
    func submitWorkbenchPrompt(
        ...
        guard validateWorkbenchSubmissionAvailability(for: mode),
              let routing = resolveWorkbenchSubmissionRouting(
                workflow: workflow,
                project: project
              ) else {
            return nil
        ...
        return submitWorkbenchChatPrompt(
            trimmedPrompt,
            workflow: workflow,
            project: project,
            routing: routing,
    ...
    @discardableResult
    private func submitWorkbenchChatPrompt(
        _ prompt: String,
        workflow: Workflow,
        project: MAProject,
    ...
    func requestStopWorkbenchConversation(_ summary: WorkbenchThreadSummary?) {
        ...
        openClawService.abortActiveRemoteConversation()
    }

    func refreshWorkbenchHistory(for workflowID: UUID? = nil, threadID: String? = nil) {
        ...
        guard let project = currentProject,
              ...
              let sessionContext = latestWorkbenchThreadContext(
                for: workflow,
                project: project,
    ...
    func latestWorkbenchThreadID(for workflowID: UUID?) -> String? {
        guard let project = currentProject,
    ...
    func workbenchThreadSummaries(for workflowID: UUID?) -> [WorkbenchThreadSummary] {
        ...
        let history = workbenchThreadHistory(for: workflow.id)

        let summaryCollections = workbenchThreadStateCoordinator.collectSummaryCollections(
            messageRecords: history.messages.compactMap { message in
                makeWorkbenchThreadSummaryMessageRecord(
                    message,
                    workflow: workflow,
                    project: project
                )
            },
            taskRecords: history.tasks.compactMap { task in
                makeWorkbenchThreadSummaryTaskRecord(
                    task,
                    workflow: workflow,
                    project: project
                )
        ...
        return workbenchThreadStateCoordinator.summarizeThreads(
            ...
            activeRunRecords: project.runtimeState.activeWorkbenchRuns,
            threadStateRecords: project.runtimeState.workbenchThreadStates
        )
        .map(WorkbenchThreadSummary.init)
    }

    private func makeWorkbenchThreadSummaryMessageRecord(
        _ message: Message,
        workflow: Workflow,
        project: MAProject
    ) -> WorkbenchThreadSummaryMessageRecord? {
        guard let threadID = resolvedWorkbenchThreadID(from: message.metadata) else { return nil }
        let contextSample = workbenchThreadContext(for: message, workflow: workflow, project: project).map {
            WorkbenchThreadContextSample(context: $0, activityAt: message.timestamp)
        }
        return WorkbenchThreadSummaryMessageRecord(
            threadID: threadID,
            message: message,
    ...
    private func makeWorkbenchThreadSummaryTaskRecord(
        _ task: Task,
        workflow: Workflow,
        project: MAProject
    ) -> WorkbenchThreadSummaryTaskRecord? {
        guard let threadID = resolvedWorkbenchThreadID(from: task.metadata) else { return nil }
        let contextSample = workbenchThreadContext(for: task, workflow: workflow, project: project).map {
            WorkbenchThreadContextSample(
                context: $0,
                activityAt: task.completedAt ?? task.startedAt ?? task.createdAt
            )
        }
        return WorkbenchThreadSummaryTaskRecord(
            threadID: threadID,
            task: task,
            contextSample: contextSample
        )
    ...
    private func makeWorkbenchSubmissionContext(
        workflow: Workflow,
        leadAgent: Agent,
        interactionMode: WorkbenchInteractionMode,
        project: MAProject,
        ...
        makeWorkbenchThreadContext(
            workflow: workflow,
            leadAgent: leadAgent,
            mode: interactionMode,
            projectSessionID: project.runtimeState.sessionID,
            project: project,
            reuseExistingThreadID: preferredThreadID
        )
    }

    private func validateWorkbenchSubmissionAvailability(
        for mode: WorkbenchInteractionMode
    ) -> Bool {
        ...
        guard canRunWorkbenchMode else {
            ...
            openClawService.addLog(.error, message)
            return false
        }
    ...
    private func resolveWorkbenchSubmissionRouting(
        workflow: Workflow,
        project: MAProject
    ) -> WorkbenchSubmissionRouting? {
        ...
        return WorkbenchSubmissionRouting(
            entryAgentNodes: entryAgentNodes,
            leadNode: leadNode,
            leadAgent: leadAgent,
    ...
    private func prepareWorkbenchSubmission(
        prompt: String,
        workflow: Workflow,
        project: MAProject,
        routing: WorkbenchSubmissionRouting,
    ...
    ) -> WorkbenchSubmissionSession {
        let threadContext = makeWorkbenchSubmissionContext(
            workflow: workflow,
        ...
        return beginWorkbenchSubmissionSession(
            prompt: prompt,
            workflow: workflow,
            leadNode: routing.leadNode,
            leadAgent: routing.leadAgent,
            entryAgentIDs: routing.entryAgentIDs,
    ...
    private func beginWorkbenchSubmissionSession(
        ...
        let submission = startWorkbenchSubmission(
            prompt: prompt,
            workflow: workflow,
            leadNode: leadNode,
            leadAgent: leadAgent,
        ...
        return WorkbenchSubmissionSession(
            startedAt: startedAt,
            context: threadContext,
            taskID: submission.taskID,
            userRuntimeEventID: submission.userRuntimeEventID
        )
    }

    private func startWorkbenchSubmission(
        prompt: String,
        workflow: Workflow,
        leadNode: WorkflowNode,
        leadAgent: Agent,
        ...
        thinkingLevel: AgentThinkingLevel? = nil,
        at startedAt: Date = Date()
    ) -> (taskID: UUID, userRuntimeEventID: String?) {
        let taskTags: [String]
        let threadTransition: WorkbenchThreadTransition
        let runtimePayload: [String: String]

        switch mode {
        case .chat:
            taskTags = ["workbench", workflow.name]
            threadTransition = .chatSubmitted
            runtimePayload = [
                "intent": "respond",
                "summary": prompt,
        ...
        case .run:
            taskTags = ["workbench", workflow.name, "run"]
            threadTransition = .runSubmitted
            runtimePayload = [
                "intent": "execute",
                "summary": prompt,
        ...
        if let thinkingLevel {
            task.metadata["workbenchThinkingLevel"] = thinkingLevel.rawValue
        }
        ...
        userMessage.metadata[WorkbenchMetadataKey.channel] = "workbench"
        userMessage.metadata["role"] = "user"
        userMessage.metadata["kind"] = "input"
        userMessage.metadata[WorkbenchMetadataKey.workflowID] = workflow.id.uuidString
        userMessage.metadata["taskID"] = task.id.uuidString
        userMessage.metadata["tokenEstimate"] = String(estimatedTokenCount(for: prompt))
        if let thinkingLevel {
            userMessage.metadata["workbenchThinkingLevel"] = thinkingLevel.rawValue
        }
        applyWorkbenchThreadMetadata(&userMessage.metadata, context: threadContext)
        userMessage.runtimeEvent = makeWorkbenchRuntimeEvent(
            ...
            threadType: threadContext.threadType,
            threadMode: threadContext.threadMode,
        ...
        return (task.id, userMessage.runtimeEvent?.id)
    }

    private func executeWorkbenchWorkflow(
        _ workflow: Workflow,
        agents: [Agent],
        prompt: String,
        projectID: UUID,
        ...
        openClawService.executeWorkflow(
            ...
            entryNodeIDsOverride: entryNodeIDsOverride,
            preloadedResults: preloadedResults,
            precompletedNodeIDs: precompletedNodeIDs,
            agentOutputMode: agentOutputMode,
    ...
    private func workbenchTaskStatus(for results: [ExecutionResult]) -> TaskStatus {
        let failedCount = results.filter { $0.status == .failed }.count
        return results.isEmpty ? .blocked : (failedCount == 0 ? .done : .blocked)
    }
    ...
    private enum WorkbenchEntryLatencyKind {
        case firstChunk
        case firstReply
        case fullWorkflow
    }
    ...
    private func makeWorkbenchEntryReplyTracker(
        ...
        return WorkbenchEntryReplyTracker(messageID: messageID)
    }

    private func appendWorkbenchEntryStreamChunk(
        _ chunk: String,
        to tracker: WorkbenchEntryReplyTracker,
        workflowID: UUID,
        taskID: UUID,
        agent: Agent,
        context: WorkbenchThreadContext,
    ...
    private func completeWorkbenchEntryReply(
        with entryResult: ExecutionResult,
        tracker: WorkbenchEntryReplyTracker,
        workflowID: UUID,
        taskID: UUID,
        agent: Agent,
        ...
        if !visibleOutput.isEmpty {
            ...
            messageManager.updateMessage(tracker.messageID) { message in
    ...
    private func recordWorkbenchEntryLatencyIfNeeded(
        _ tracker: WorkbenchEntryReplyTracker,
        taskID: UUID,
        key: String,
        label: String,
        ...
        sessionID: String,
        agentID: UUID,
        kind: WorkbenchEntryLatencyKind,
        at timestamp: Date = Date()
    ) {
        switch kind {
        case .firstChunk:
            guard tracker.firstChunkAt == nil else { return }
            tracker.firstChunkAt = timestamp
        case .firstReply:
            guard tracker.firstReplyAt == nil else { return }
            tracker.firstReplyAt = timestamp
        case .fullWorkflow:
            guard tracker.fullWorkflowAt == nil else { return }
            tracker.fullWorkflowAt = timestamp
        }
    ...
    private func recordWorkbenchLatency(
        taskID: UUID,
        messageID: UUID?,
        key: String,
        label: String,
        ...
        let latency = latencyMillisecondsString(since: startedAt, until: timestamp)
        persistWorkbenchLatencyMetric(taskID: taskID, messageID: messageID, key: key, value: latency)
        openClawService.addLog(
            .info,
    ...
    private func latestWorkbenchThreadContext(
        ...
        if let preferredThreadID,
           let context = resolvedWorkbenchThreadContext(
                for: workflow,
                project: project,
        ...
        return resolvedWorkbenchThreadContext(for: workflow, project: project)
    }
    ...
    private func workbenchThreadContext(
        ...
        resolvedWorkbenchThreadContext(
            for: workflow,
            project: project,
            threadID: threadID
        )
    ...
    private func resolvedWorkbenchThreadContext(
        for workflow: Workflow,
        project: MAProject,
        threadID: String? = nil
    ) -> WorkbenchRemoteSessionContext? {
        let history = workbenchThreadHistory(for: workflow.id, threadID: threadID)
        let messageContext = firstResolvedWorkbenchThreadContext(
            from: history.messages
        ) { message in
            workbenchThreadContext(for: message, workflow: workflow, project: project)
        }
        return messageContext ?? firstResolvedWorkbenchThreadContext(
            from: history.tasks
        ) { task in
            workbenchThreadContext(for: task, workflow: workflow, project: project)
        }
    }
    ...
    private func workbenchThreadContext(
        ...
        resolveWorkbenchRemoteSessionContext(
            workflow: workflow,
            metadata: message.metadata,
            leadAgentCandidateIDs: [message.fromAgentID, message.toAgentID],
            project: project
        )
    }
    ...
    private func workbenchThreadContext(
        ...
        resolveWorkbenchRemoteSessionContext(
            ...
            leadAgentCandidateIDs: [task.assignedAgentID],
            project: project
        )
    }

    private func resolveWorkbenchRemoteSessionContext(
        workflow: Workflow,
        metadata: [String: String],
        leadAgentCandidateIDs: [UUID?],
        project: MAProject
    ) -> WorkbenchRemoteSessionContext? {
        resolveWorkbenchRemoteSessionContext(
            workflow: workflow,
            metadata: metadata,
            leadAgent: workbenchLeadAgent(
                candidateIDs: leadAgentCandidateIDs + workbenchEntryAgentCandidateIDs(from: metadata),
                project: project
            ),
            defaultProjectSessionID: project.runtimeState.sessionID
        )
    }
    ...
    private func resolveWorkbenchRemoteSessionContext(
        ...
        guard let threadID = resolvedWorkbenchThreadID(from: metadata),
              let sessionID = resolvedWorkbenchSessionID(from: metadata),
              let leadAgent else {
            return nil
        }

        return makeWorkbenchThreadContext(
            workflow: workflow,
            sessionID: sessionID,
            threadID: threadID,
            interactionMode: WorkbenchInteractionMode(normalizedRawValue: metadata[WorkbenchMetadataKey.workbenchMode])
                ?? .chat,
            threadType: resolvedWorkbenchThreadType(from: metadata) ?? .conversationAutonomous,
            threadMode: resolvedWorkbenchThreadMode(from: metadata) ?? .autonomousConversation,
            entryAgent: leadAgent,
            projectSessionID: metadata[WorkbenchMetadataKey.workbenchProjectSessionID]
    ...
    private func workbenchMessages(
        ...
        let query = WorkbenchRecordQuery(
            workflowID: workflowID,
            threadID: threadID,
            requiresWorkbenchChannel: true
        )
        return filteredWorkbenchRecords(
            messageManager.messages,
            query: query,
            metadata: \.metadata
        )
    ...
    private func filteredWorkbenchRecords<Record>(
        _ records: [Record],
        ...
        records
            .filter { record in
                query.matches(metadata: metadata(record))
            }
    }
    ...
    private func filteredWorkbenchRecords<Record>(
        _ records: [Record],
        query: WorkbenchRecordQuery,
        metadata: KeyPath<Record, [String: String]>
    ) -> [Record] {
        filteredWorkbenchRecords(records, query: query) { record in
            record[keyPath: metadata]
        }
    }
    ...
    private func workbenchMessages(
        ...
        workbenchThreadHistory(for: context).messages
    }
    ...
    private func workbenchTasks(
        ...
        let query = WorkbenchRecordQuery(
            workflowID: workflowID,
            threadID: threadID,
            requiresWorkbenchChannel: false
        )
        return filteredWorkbenchRecords(
            taskManager.tasks,
            query: query,
    ...
    private func workbenchTasks(
        ...
        workbenchThreadHistory(for: context).tasks
    }

    private func workbenchThreadHistory(
        for workflowID: UUID,
        threadID: String? = nil
    ) -> WorkbenchThreadHistory {
        WorkbenchThreadHistory(
            messages: workbenchMessages(for: workflowID, threadID: threadID),
            tasks: workbenchTasks(for: workflowID, threadID: threadID)
        )
    }
    ...
    private func workbenchThreadHistory(
        ...
        workbenchThreadHistory(for: context.workflowID, threadID: context.threadID)
    }

    private func firstResolvedWorkbenchThreadContext<Record>(
        from records: [Record],
        resolve: (Record) -> WorkbenchRemoteSessionContext?
    ) -> WorkbenchRemoteSessionContext? {
        for record in records {
            guard let context = resolve(record) else {
                continue
            }
            return context
        }
    ...
    private func makeWorkbenchThreadContext(
        ...
        if let reuseExistingThreadID,
           let existingContext = workbenchThreadContext(for: reuseExistingThreadID, workflow: workflow, project: project) {
            let reusesExistingMode = existingContext.interactionMode == mode
            return makeWorkbenchThreadContext(
                workflow: workflow,
                sessionID: existingContext.sessionID,
                threadID: existingContext.threadID,
                interactionMode: mode,
        ...
        let normalizedProjectSessionID = projectSessionID.trimmingCharacters(in: .whitespacesAndNewlines)
        let resolvedProjectSessionID = normalizedProjectSessionID.isEmpty ? UUID().uuidString : normalizedProjectSessionID
        let threadID = workbenchThreadID(
            projectRuntimeSessionID: resolvedProjectSessionID,
            workflowID: workflow.id,
            mode: mode
        )
        let sessionID = workbenchSessionID(
            projectRuntimeSessionID: resolvedProjectSessionID,
            workflowID: workflow.id,
            agentID: leadAgent.id,
            mode: mode,
        ...
        return makeWorkbenchThreadContext(
            ...
            entryAgent: leadAgent,
            projectSessionID: resolvedProjectSessionID
        )
    }
    ...
    private func makeWorkbenchThreadContext(
        ...
        return WorkbenchThreadContext(
            ...
            threadMode: threadMode,
            executionIntent: interactionMode.executionIntent,
            origin: interactionMode == .chat ? "workbench_chat" : "workbench_run",
            agentID: entryAgent.id,
            agentName: entryAgent.name
        )
    }

    private func workbenchEntryAgentCandidateIDs(
        from metadata: [String: String]
    ) -> [UUID?] {
        [
            UUID(uuidString: metadata[WorkbenchMetadataKey.workbenchEntryAgentID] ?? ""),
            UUID(uuidString: metadata["entryAgentID"] ?? "")
        ]
    }

    private func workbenchLeadAgent(
        candidateIDs: [UUID?],
        project: MAProject
    ) -> Agent? {
    ...
    private func mergeWorkbenchTranscript(
        ...
        var existingDigests = existingWorkbenchTranscriptDigests(for: sessionContext)

        for entry in remoteMessages {
            let role = entry.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
            let timestamp = normalizedWorkbenchTranscriptDate(entry.timestamp)
            let digest = workbenchTranscriptDigest(role: role, text: entry.text)
            if existingDigests.contains(digest) {
                continue
            }

            messageManager.appendMessage(
                makeWorkbenchTranscriptMessage(
                    role: role,
                    text: entry.text,
                    digest: digest,
                    timestamp: timestamp,
    ...
    private func existingWorkbenchTranscriptDigests(
        for sessionContext: WorkbenchRemoteSessionContext
    ) -> Set<String> {
        Set(
            workbenchMessages(for: sessionContext).compactMap { message in
                workbenchTranscriptDigest(from: message)
            }
        )
    }

    private func workbenchTranscriptDigest(
        ...
        return workbenchTranscriptDigest(role: role, text: message.content)
    }

    private func makeWorkbenchTranscriptMessage(
        role: String,
        text: String,
        digest: String,
        timestamp: Date,
        ...
        message.metadata[WorkbenchMetadataKey.workflowID] = sessionContext.workflowID.uuidString
        message.metadata["remoteTranscriptDigest"] = digest
        applyWorkbenchThreadMetadata(&message.metadata, context: sessionContext)
        if role == "assistant" {
            message.metadata["entryReply"] = "true"
        ...
        message.runtimeEvent = makeTranscriptRuntimeEvent(
            role: role,
            text: text,
            sessionContext: sessionContext
        )
    ...
    private func persistWorkbenchGatewaySessionKey(
        ...
        let history = workbenchThreadHistory(for: context)
        let messageIDs = history.messages.map(\.id)
    ...
    private func applyWorkbenchAssistantMessageMetadata(
        ...
        message.metadata["kind"] = "output"
        message.metadata[WorkbenchMetadataKey.workflowID] = workflowID.uuidString
        message.metadata["taskID"] = taskID.uuidString
        if isEntryReply {
            message.metadata["entryReply"] = "true"
        } else {
            message.metadata.removeValue(forKey: "entryReply")
        }
        if isStreamed {
            message.metadata["streamed"] = "true"
        } else {
            message.metadata.removeValue(forKey: "streamed")
        }
        ...
        if let thinkingLevel {
            message.metadata["workbenchThinkingLevel"] = thinkingLevel.rawValue
        } else {
            message.metadata.removeValue(forKey: "workbenchThinkingLevel")
        }
        message.metadata["tokenEstimate"] = String(estimatedTokenCount(for: message.content))
        applyWorkbenchThreadMetadata(&message.metadata, context: context)
        if let runtimeEvent {
            message.runtimeEvent = runtimeEvent
        }
    }

    private func workbenchTranscriptDigest(role: String, text: String) -> String {
    ...
    private func applyWorkbenchThreadMetadata(
        ...
        metadata[WorkbenchMetadataKey.workflowID] = context.workflowID.uuidString
        metadata[WorkbenchMetadataKey.workbenchSessionID] = context.sessionID
    ...
    private func makeTranscriptRuntimeEvent(
        ...
        return OpenClawRuntimeEvent(
            ...
            control: [
                "executionIntent": sessionContext.executionIntent.rawValue,
                "workbenchThreadType": sessionContext.threadType.rawValue,
                "workbenchThreadMode": sessionContext.threadMode.rawValue
            ]
        )
    }
    ...
    private func workbenchThreadID(
        ...
        return "thread-\(resolvedBase)-\(workflowID.uuidString)-\(mode.rawValue)-\(UUID().uuidString)"
    }

    private func workbenchSessionID(
        projectRuntimeSessionID: String,
        workflowID: UUID,
        agentID: UUID,
        mode: WorkbenchInteractionMode,
        ...
        let base = projectRuntimeSessionID.trimmingCharacters(in: .whitespacesAndNewlines)
        let resolvedBase = base.isEmpty ? UUID().uuidString : base
        let threadToken = threadID.components(separatedBy: "-").last ?? UUID().uuidString
        return "workbench-\(resolvedBase)-\(workflowID.uuidString)-\(mode.rawValue)-\(agentID.uuidString)-\(threadToken)"
    }
